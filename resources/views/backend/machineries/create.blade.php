...
@section('header_buttons')
    <div class="row">
        <div class="col-sm-12 d-flex justify-content-end mb-3">
            <a href="{{ url('admin/machineries') }}" class="btn btn-primary">
...
@section('content')
    ...
    <form method="POST" action="{{ route('backend.machineries.store') }}">
        @csrf
        <input type="hidden" name="id" value="{{ isset($machinery) ? $machinery->id : '' }}">
        <input type="hidden" id="temp_id" name="temp_id" value="{{ $temp_id }}">
        <div class="row">
            <div class="col-sm-7">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Machinery Details</h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-12 mb-4">
                                <label>Slug</label>
                                <input class="form-control" type="text" id="slug" name="slug" placeholder="Enter here...." value="{{ isset($machinery) ? $machinery->slug : '' }}" readonly>
                                <label class="text-danger fw-bold mt-1 d-none" id="slug-warning">Slug already exists!</label>
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Machinery Title *</label>
                                <input class="form-control" type="text" id="main-title" name="machinery" placeholder="Enter here...." value="{{ isset($machinery) ? $machinery->machinery : '' }}">
                            </div>

                            <div class="col-sm-12 mb-4">
                                <label>Description</label>
                                <textarea id="content-en" name="description">
                                    {{ isset($machinery) ? $machinery->description : '' }}
                                </textarea>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-sm-8">
                                ...
                                <select class="form-control" name="machinery_category_id">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ (!empty($machinery) && $machinery->machinery_category_id == $category->id) ? 'selected' : '' }}>{{ $category->machinery_category }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-sm-4">
                                <label>Display Order</label>
                                <input class="form-control" type="text" name="display_order" placeholder="Enter here...." value="{{ isset($machinery) ? $machinery->display_order : '' }}">
                            </div>
                        </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-sm-12 mb-4" id="labels-area">
                                ...
                                @if(!empty($machinery) && !empty($machinery_labels))
                                    @foreach($machinery_labels as $pl)
    ...
    <input type="hidden" id="route_get_slug" value="{{ route('backend.machineries.slugGenerator') }}">
    ...
    <script>
        ...
        $('.thumb_crop').click(function(event){
            ...
            $image_thumb.croppie('result', {
                ...
                $.ajax({
                    url: "{{ route('backend.machineries.imageUpload') }}",
                    type: "POST",
                    data: {
                        image:response,
                        id: $id,
        ...
        $(document).ready(function (){
            ...
            $('#uploaded_image').on('click', '.delete-image', function ($e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            $.ajax({
                                url: "{{ route('backend.machineries.deleteImage') }}",
                                type: 'POST',
                                data: {
                                    id: $id,
                                    _token: csrf_token()
            ...
            $('#uploaded_image').on('click', '.primary-image', function ($e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            $.ajax({
                                url: "{{ route('backend.machineries.setPrimaryImage') }}",
                                type: 'POST',
                                data: {
                                    id: $id,
                                    _token: csrf_token()
            ...
            $('.add-new-label-row').on('click', function ($e){
                $.ajax({
                    url: "{{ route('backend.machineries.getDetailsForPriceRow') }}",
                    type: 'POST',
                    data: {
                        _token: csrf_token()
                    },
